...
// @downloadURL  https://raw.githubusercontent.com/Parkinwad/Steam-Economy-Enhancer-2/master/code.user.js
// @updateURL    https://raw.githubusercontent.com/Parkinwad/Steam-Economy-Enhancer-2/master/code.user.js
// ==/UserScript==
...
(function ($, async) {
    ...
    const GAMES_WITH_CARDS = [
        ...
        { appid: 570, name: 'Dota 2', type: 'card', cards: [] },
        { appid: 440, name: 'Team Fortress 2', type: 'card', cards: [] },
        { appid: 252490, name: 'Rust', type: 'card', cards: [] },
        { appid: 578080, name: 'PUBG', type: 'card', cards: [] },
        { appid: 271590, name: 'Grand Theft Auto V', type: 'card', cards: [] },
        { appid: 359550, name: "Tom Clancy's Rainbow Six Siege", type: 'card', cards: [] },
        { appid: 230410, name: 'Warframe', type: 'card', cards: [] },
    ];
    // Booster pack app IDs to scan
    const BOOSTER_PACKS = [
        { appid: 730, contextid: '6', name: 'CS2 Booster Pack', type: 'booster' },
        { appid: 570, contextid: '6', name: 'Dota 2 Booster Pack', type: 'booster' },
        { appid: 440, contextid: '2', name: 'TF2 Booster Pack', type: 'booster' },
    ];
    // Calculate expected value of a booster pack
    function calculateBoosterPackExpectedValue(appid, contextid) {
        // This is a conservative estimate based on typical booster pack contents
        // Steam doesn't expose pack contents directly, so we use statistical averages
        const cardsPerPack = 3; // Average cards per pack
        const rareChance = 0.1; // 10% chance of rare card
        const foilChance = 0.001; // 0.1% chance of foil
        
        // Get average card price for this game (simplified - in reality should cache this)
        let avgCardPrice = 0.05; // Default conservative estimate in dollars
        
        // Calculate expected value
        const expectedCardValue = cardsPerPack * avgCardPrice;
        const expectedRareValue = rareChance * (avgCardPrice * 10); // Rare cards ~10x value
        const expectedFoilValue = foilChance * (avgCardPrice * 50); // Foils ~50x value
        
        const totalExpectedValue = expectedCardValue + expectedRareValue + expectedFoilValue;
        
        // Convert to cents (before fees)
        return totalExpectedValue * 100;
    }
    // Check if an item is a good deal
    function evaluateDeal(item, marketData) {
        const currentPrice = marketData.lowest_sell_order;
        if (!currentPrice || currentPrice <= 0) return null;
        
        let expectedSellPrice = 0;
        let minProfitCents = 0;
        
        if (item.type === 'card') {
            // For cards, use the buy order price or estimated sell price
            expectedSellPrice = marketData.highest_buy_order || calculateEstimatedSellPrice(marketData);
            minProfitCents = getSettingWithDefault(SETTING_DEAL_MIN_PROFIT_CENTS) * 100;
        } else if (item.type === 'booster') {
            // For booster packs, use expected value minus fees
            const expectedValue = calculateBoosterPackExpectedValue(item.appid, item.contextid);
            // Apply conservative fee estimate (Steam 15% + publisher ~10%)
            const fees = expectedValue * 0.25; // 25% conservative fee estimate
            expectedSellPrice = expectedValue - fees;
            minProfitCents = getSettingWithDefault(SETTING_DEAL_BOOSTER_MIN_PROFIT_CENTS) * 100;
        }
        
        if (expectedSellPrice <= 0) return null;
        
        // Calculate profit
        const profit = expectedSellPrice - currentPrice;
        const profitPercent = (profit / currentPrice) * 100;
        
        // Only return if profit meets minimum threshold
        if (profit >= minProfitCents) {
            return {
                item: item,
    ...
    // Calculate estimated sell price for cards when no buy order exists
    function calculateEstimatedSellPrice(marketData) {
        // Use a conservative estimate based on listing data
        if (marketData.lowest_sell_order) {
            // Estimate 95% of current lowest listing (conservative)
            return marketData.lowest_sell_order * 0.95;
        }
        return 0;
    }
    // Scan market for deals
    async function scanMarketForDeals() {
        const maxItems = getSettingWithDefault(SETTING_DEAL_MAX_ITEMS_PER_SCAN);
        const maxSpend = getSettingWithDefault(SETTING_DEAL_MAX_SPEND_PER_SCAN) * 100; // Convert to cents
        const results = [];
        
        document.getElementById('deal_scanner_status').textContent = 'Scanning...';
        
        // Scan cards
        for (const game of GAMES_WITH_CARDS) {
            if (totalSpentOnScans >= maxSpend) break;
            
            try {
                const marketData = await getMarketData(game.appid);
                if (!marketData || !marketData.lowest_sell_order) continue;
                
                const deal = evaluateDeal(
                    { appid: game.appid, contextid: '6', name: game.name + ' Trading Card', type: 'card' },
                    marketData
                );
                
                if (deal) {
                    results.push(deal);
                    totalSpentOnScans += deal.currentPrice;
                }
                
                // Conservative delay between scans
                await new Promise(resolve => setTimeout(resolve, getSettingWithDefault(SETTING_DEAL_SCAN_DELAY_MS)));
            } catch (error) {
                console.error('Error scanning game ' + game.name + ':', error);
            }
        }
        ...
        for (const pack of BOOSTER_PACKS) {
            ...
            try {
                const marketData = await getMarketData(pack.appid, pack.contextid);
                if (!marketData || !marketData.lowest_sell_order) continue;
                
                const deal = evaluateDeal(pack, marketData);
                ...
                console.error('Error scanning booster pack ' + pack.name + ':', error);
            }
        }
    ...
    async function getMarketData(appid, contextid) {
        contextid = contextid || '6';
        return new Promise((resolve) => {
            const url = window.location.origin + '/market/listings/' + appid + '/market_data/json?market_hash_name=';
            
            // For demo purposes, we'll use a placeholder
            // In reality, you'd need to query the Steam market API for each item
            resolve({
                lowest_sell_order: 0,
                highest_buy_order: 0,
                buy_order_graph: [],
    ...
    // Display deals to user
    function displayDeals(deals) {
        const resultsDiv = document.getElementById('deal_scanner_results');
        resultsDiv.style.display = 'block';
        resultsDiv.innerHTML = '';
        
        if (deals.length === 0) {
            resultsDiv.innerHTML = '<div style="color: #767676; padding: 8px;">No deals found matching your criteria.</div>';
            document.getElementById('deal_scanner_status').textContent = 'No deals found';
            return;
        }
        ...
        deals.forEach((deal, index) => {
            const profitColor = deal.profitPercent > 50 ? '#407736' : '#908F44';
            html += '<div style="border: 1px solid #444; padding: 8px; margin-bottom: 8px; background: #1a1a1a;">';
            html += '<div style="font-weight: bold; color: #ebebeb;">' + deal.item.name + '</div>';
            html += '<div style="color: #767676; font-size: 11px;">Current Price: ' + formatPrice(deal.currentPrice) + '</div>';
            html += '<div style="color: #767676; font-size: 11px;">Expected Sell: ' + formatPrice(deal.expectedSellPrice) + '</div>';
            html += '<div style="color: ' + profitColor + '; font-size: 11px;">Profit: ' + formatPrice(deal.profit) + ' (' + deal.profitPercent.toFixed(1) + '%)</div>';
            html += '<button class="btn_darkblue_white_innerfade btn_small" style="margin-top: 4px; padding: 2px 8px;" onclick="purchaseDeal(' + index + ')">Purchase</button>';
            html += '</div>';
        });
        
        resultsDiv.innerHTML = html;
        document.getElementById('deal_scanner_status').textContent = 'Found ' + deals.length + ' deal(s)';
    }
    // Purchase a deal
    function purchaseDeal(index) {
        const deal = dealsFound[index];
        if (!deal) return;
        
        // Confirm with user
        const confirmed = confirm('Purchase ' + deal.item.name + ' for ' + formatPrice(deal.currentPrice) + '?\n\nExpected profit: ' + formatPrice(deal.profit) + ' (' + deal.profitPercent.toFixed(1) + '%)');
        
        if (confirmed) {
            // Place buy order through Steam
            const url = window.location.origin + '/market/createbuyorder/';
            const options = {
                method: 'POST',
                data: {
                    appid: deal.item.appid,
                    market_hash_name: deal.item.name,
                    price: Math.floor(deal.currentPrice)
                }
            };
            
            request(url, options, (error, data) => {
                if (error || !data.success) {
                    logDOM('Failed to purchase ' + deal.item.name + ': ' + (error || 'Unknown error'));
                    alert('Purchase failed. Please try again.');
                } else {
                    logDOM('Successfully purchased ' + deal.item.name + ' for ' + formatPrice(deal.currentPrice) + '!');
                    alert('Successfully purchased ' + deal.item.name + '!\n\nIt will appear in your market buy orders.');
                    
                    // Remove from results
                    dealsFound.splice(index, 1);
                    if (dealsFound.length > 0) {
                        displayDeals(dealsFound);
                    } else {
                        document.getElementById('deal_scanner_results').style.display = 'none';
                    }
                }
            });
        }
    }
    // Initialize deal scanner event handlers
    function initDealScanner() {
        const scanButton = document.getElementById('deal_scanner_run');
        if (!scanButton) return;
        
        scanButton.addEventListener('click', async () => {
            if (dealScannerActive) {
                alert('A scan is already in progress. Please wait.');
                return;
            }
            
            dealScannerActive = true;
            totalSpentOnScans = 0;
            document.getElementById('deal_scanner_status').textContent = 'Scanning...';
            document.getElementById('deal_scanner_results').style.display = 'none';
            
            try {
                dealsFound = await scanMarketForDeals();
                displayDeals(dealsFound);
            } catch (error) {
    ...
    if (currentPage == PAGE_INVENTORY) {
        ...
        function updateInventoryUI(isOwnInventory) {
            ...
            setSetting(SETTING_DEAL_SCAN_DELAY_MS, $(`#${SETTING_DEAL_SCAN_DELAY_MS}`).val());            window.location.reload();
                }
            );
